...
export default function NaylaCore() {
  ...
  // ESTADOS DEL MOTOR Y COMPONENTES
  ...
  // REORDENAMIENTO ARRASTRABLE (DRAG & DROP)
  ...
  const globalStyles = `
    ...
    .clip-block { flex-shrink: 0; height: 60px; width: 100px; border-radius: 12px; background: #1a1a1a; border: 2px solid transparent; overflow: hidden; position: relative; cursor: grab; display: flex; justify-content: center; align-items: center; }
    .clip-block.selected { border-color: #ffffff; }
    .clip-block:active { cursor: grabbing; opacity: 0.8; }
    .audio-block { flex-shrink: 0; height: 40px; width: 150px; border-radius: 12px; background: #0a1526; border: 1px solid #0055ff; display: flex; align-items: center; justify-content: space-between; padding: 0 10px; cursor: pointer; }
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#000000', color: '#ededed', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', borderBottom: '1px solid #1a1a1a' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <span style={{ fontSize: '0.65rem', color: '#737373', letterSpacing: '1px' }}>ID: {session.user.email}</span>
        </div>
      </header>

      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        
        {/* MONITOR CENTRAL CON ADAPTACIÓN DE PROPORCIÓN DINÁMICA */}
        <section style={{ flex: 1, backgroundColor: '#050505', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '10px', overflow: 'hidden' }}>
          <div style={{ 
            ...
            backgroundColor: '#0a0a0a', borderRadius: '24px', overflow: 'hidden', position: 'relative',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 20px', alignItems: 'center', borderBottom: '1px solid #1a1a1a' }}>
          ...
          <span style={{ color: '#737373', fontSize: '0.75rem' }}>{message ? 'RESTRICCIÓN DE PROCESO' : 'LÍNEA ACTIVA'}</span>
        </div>

        {/* LÍNEA DE TIEMPO EN CASCADA (TIMELINE TRACKS) */}
        <section style={{ height: '220px', backgroundColor: '#000000', padding: '15px', display: 'flex', flexDirection: 'column', gap: '5px', overflowY: 'auto' }}>
          
          {/* TRACK CENTRAL DE VIDEO */}
          <div className="timeline-track">
            <div style={{ flexShrink: 0, width: '40px', height: '60px', background: '#ffffff', borderRadius: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#000', fontSize: '1.2rem', fontWeight: 'bold' }}>+</div>
            {pistaVideo.length === 0 ? <p style={{ color: '#262626', fontSize: '0.7rem', marginLeft: '10px' }}>Bodega vacía. Envía recursos desde abajo.</p> : null}
            ...
            {pistaVideo.map((item, index) => (
              ...
              >
                ...
                <span style={{ position: 'absolute', bottom: '4px', left: '6px', fontSize: '0.55rem', color: '#fff', fontWeight: 'bold' }}>{item.etiqueta}</span>
              </div>
            ))}
          </div>

          {/* TRACK SECUNDARIO DE AUDIO (ONDAS DE FRECUENCIA BLANCAS) */}
          <div className="timeline-track" style={{ minHeight: '50px' }}>
            <div style={{ flexShrink: 0, width: '40px', height: '40px', background: '#1a1a1a', borderRadius: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#737373', fontSize: '1rem' }}>🎵</div>
            {pistaAudio.map(item => (
              <div 
                key={item.id} 
                onClick={() => setClipSeleccionado(item.id)}
              ...
              >
                ...
                {/* Ondas lineales vectoriales puras en blanco */}
                <div style={{ display: 'flex', gap: '2px', alignItems: 'center' }}>
                  {[...Array(8)].map((_, i) => (
                    <div key={i} style={{ width: '2px', height: `${Math.random() * 18 + 4}px`, backgroundColor: '#ffffff', borderRadius: '2px' }} />
                  ))}
                </div>
              </div>
            ))}
        ...
        {/* MENÚ DE ACCIONES Y CONTEXTO INFERIOR */}
        <section style={{ borderTop: '1px solid #1a1a1a', padding: '15px', backgroundColor: '#050505', display: 'flex', overflowX: 'auto', gap: '10px' }}>
          {clipSeleccionado ? (
            <>
              <button onClick={() => setClipSeleccionado(null)} className="btn-capsule">← REGRESAR</button>
              <button className="btn-capsule">✂️ FRAGMENTAR</button>
              <button className="btn-capsule">⏱️ ACELERACIÓN</button>
              <button onClick={() => {
              ...
              }} style={{ background: 'transparent', border: '1px solid #ff4444', color: '#ff4444', borderRadius: '100px', padding: '0.8rem 1.2rem', fontSize: '0.7rem', fontWeight: 'bold' }}>Eliminar Elemento</button>
            </>
          ) : (
            <>
              <button className={`btn-capsule ${navActiva === 'galeria' ? 'active' : ''}`} onClick={() => setNavActiva('galeria')}>📁 BODEGA MULTIPLE</button>
              <button className={`btn-capsule ${navActiva === 'script' ? 'active' : ''}`} onClick={() => setNavActiva('script')}>⚡ SCRIPT CODE (JS)</button>
              <button className={`btn-capsule ${navActiva === 'ia' ? 'active' : ''}`} onClick={() => setNavActiva('ia')}>🤖 INTERFAZ COGNITIVA</button>
            </>
          )}
        </section>

        {/* PANELES DE ENLACE DINÁMICO */}
        {!clipSeleccionado && navActiva === 'galeria' && (
          <div style={{ position: 'absolute', bottom: '80px', left: 0, right: 0, height: '40vh', backgroundColor: '#000', borderTop: '1px solid #262626', padding: '1rem', zIndex: 100, overflowY: 'auto', borderTopLeftRadius: '24px', borderTopRightRadius: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <span style={{ fontSize: '0.75rem', color: '#737373', letterSpacing: '1px' }}>RECURSOS DE COMPILACIÓN DE ORIGEN</span>
              <button onClick={() => setNavActiva(null)} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '1.2rem' }}>×</button>
            </div>
            
            <div style={{ display: 'flex', gap: '10px', marginBottom: '1rem' }}>
              <label style={{ flex: 1, textAlign: 'center', padding: '1rem', border: '1px dashed #404040', borderRadius: '16px', fontSize: '0.7rem', color: '#a3a3a3', cursor: 'pointer' }}>+ VIDEos <input type="file" multiple accept="video/*" onChange={(e) => handleSubirMultimedia(e, 'video')} style={{ display: 'none' }} /></label>
              <label style={{ flex: 1, textAlign: 'center', padding: '1rem', border: '1px dashed #404040', borderRadius: '16px', fontSize: '0.7rem', color: '#a3a3a3', cursor: 'pointer' }}>+ FOTOs <input type="file" multiple accept="image/*" onChange={(e) => handleSubirMultimedia(e, 'foto')} style={{ display: 'none' }} /></label>
              <label style={{ flex: 1, textAlign: 'center', padding: '1rem', border: '1px dashed #404040', borderRadius: '16px', fontSize: '0.7rem', color: '#a3a3a3', cursor: 'pointer' }}>+ AUDIOS <input type="file" multiple accept="audio/*" onChange={(e) => handleSubirMultimedia(e, 'audio')} style={{ display: 'none' }} /></label>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
              {galeria.map(item => (
                <div key={item.id} onClick={() => enviarAPista(item)} style={{ aspectRatio: '1', backgroundColor: '#1a1a1a', borderRadius: '12px', overflow: 'hidden', position: 'relative', cursor: 'pointer' }}>
                  {item.tipo === 'video' || item.tipo === 'foto' ? (
                    <img src={item.url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  ) : (
                    <div style={{ display: 'flex', height: '100%', justifyContent: 'center', alignItems: 'center', color: '#fff', fontSize: '1.5rem' }}>🎵</div>
                  )}
                  <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', opacity: 0, transition: '0.2s' }} onMouseOver={e => e.currentTarget.style.opacity = '1'} onMouseOut={e => e.currentTarget.style.opacity = '0'}>
                    <span style={{ color: '#fff', fontSize: '0.65rem', fontWeight: 'bold', letterSpacing: '1px' }}>MONTAR</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* INYECTOR JS CONEXIÓN NATIVA AL SERVIDOR ALPINE (ORACLE) */}
        {!clipSeleccionado && navActiva === 'script' && (
          <div style={{ position: 'absolute', bottom: '80px', left: 0, right: 0, height: '40vh', backgroundColor: '#050505', borderTop: '1px solid #262626', padding: '1.5rem', zIndex: 100, borderTopLeftRadius: '24px', borderTopRightRadius: '24px' }}>
             <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
              <span style={{ fontSize: '0.75rem', color: '#737373' }}>INYECTOR DIRECTO AL PROCESADOR</span>
              <button onClick={() => setNavActiva(null)} style={{ background: 'none', border: 'none', color: '#fff' }}>×</button>
            </div>
            <textarea value={codigoJsInput} onChange={(e) => setCodigoJsInput(e.target.value)} style={{ width: '100%', height: '55%', backgroundColor: '#000', border: '1px solid #262626', color: '#00ffcc', padding: '1rem', fontFamily: 'monospace', borderRadius: '16px', outline: 'none' }} />
            
            <button 
              onClick={async () => {
                if (pistaVideo.length === 0) return alert("Construye una secuencia en la pista primero.");
                setIsProcessing(true);
                try {
                  const res = await fetch('/api/render', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ pistaVideo })
                  });
                  const data = await res.json();
                  if (data.success) {
                    setVideoTerminado(data.url);
                    alert("Renderización unificada completada en el servidor.");
                  } else {
                    alert("Excepción del procesador: " + data.error);
                  }
                } catch (error) {
                  alert("Fallo de transmisión de línea.");
                } finally {
                  setIsProcessing(false);
                }
              }}
              disabled={isProcessing}
              style={{ width: '100%', padding: '1rem', backgroundColor: isProcessing ? '#262626' : '#fff', color: isProcessing ? '#737373' : '#000', border: 'none', borderRadius: '100px', fontWeight: 'bold', marginTop: '0.8rem', cursor: isProcessing ? 'wait' : 'pointer', fontSize: '0.8rem' }}
            >
              {isProcessing ? 'COMPILANDO SECUENCIAS MULTITRACK EN SERVIDOR...' : 'EJECUTAR EN MOTOR'}
            </button>
          </div>
        )}
