...
export default function NaylaCore() {
  ...
  // FLUJO DE INGRESO BIOMÉTRICO (CUANDO YA ESTÁ REGISTRADA)
  ...
  // FLUJO DE REGISTRO BIOMÉTRICO (NUEVO - PARA VINCULAR LA HUELLA)
  const handleRegistrarBiometria = async () => {
    try {
      // 1. Iniciar la inscripción del WebAuthn
      const { data, error } = await (supabase.auth as any).mfa.enroll({
        factorType: 'webauthn'
      });
      
      if (error) {
        alert('Error del sensor: ' + error.message);
        return;
      }
      
      // 2. Levantar el escáner del dispositivo (Huella/Iris)
      const { data: challengeData, error: challengeError } = await (supabase.auth as any).mfa.challenge({
        factorId: data.id
      });
      
      if (challengeError) {
        alert('Validación cancelada: ' + challengeError.message);
        return;
      }
      
      // 3. Confirmar la vinculación en la base de datos
      ...
      if (verifyError) {
        alert('Error final de servidor: ' + verifyError.message);
        return;
      }
      
      alert("✅ IDENTIDAD BIOMÉTRICA VINCULADA. Tu huella ha sido registrada como llave maestra del sistema.");
    } catch (err: any) {
      alert("Fallo al invocar el escáner del dispositivo.");
    }
  };
  ...
  const handleOtpVerify = async (e: React.FormEvent) => {
    ...
    setAuthLoading(true);
    setMessage('Verificando código maestro...');
    try {
      ...
      setMessage(`Error en la base de datos central.`);
    } finally {
      setAuthLoading(false);
    }
  };
          ...
          {!otpEnviado ? (
            <form onSubmit={handleEmailAuth} style={{ textAlign: 'left' }}>
              ...
              <button type="submit" disabled={authLoading} style={{ width: '100%', padding: '0.8rem', backgroundColor: '#000000', color: '#ffffff', border: '1px solid #ffffff', borderRadius: '2px', fontWeight: 'bold', fontSize: '0.75rem', letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase' }}>
                {authLoading ? 'ENVIANDO...' : 'SOLICITAR CÓDIGO DE ACCESO'}
              </button>
            </form>
          ) : (
            <form onSubmit={handleOtpVerify} style={{ textAlign: 'left' }}>
              <label style={{ fontSize: '0.7rem', color: '#737373', textTransform: 'uppercase', letterSpacing: '1px', display: 'block', marginBottom: '0.5rem', textAlign: 'center' }}>Código recibido en {emailInput}</label>
              <input type="text" placeholder="123456" value={otpInput} onChange={(e) => setOtpInput(e.target.value)} maxLength={6} style={{ width: '100%', padding: '1.2rem', backgroundColor: '#000000', border: '1px solid #ffffff', borderRadius: '2px', color: '#ffffff', fontSize: '1.8rem', marginBottom: '1rem', textAlign: 'center', letterSpacing: '0.5rem' }} />
              <button type="submit" disabled={authLoading || otpInput.length < 6} style={{ width: '100%', padding: '1rem', backgroundColor: '#ffffff', color: '#000000', border: 'none', borderRadius: '2px', fontWeight: 'bold', fontSize: '0.85rem', letterSpacing: '1px', cursor: (authLoading || otpInput.length < 6) ? 'not-allowed' : 'pointer', textTransform: 'uppercase' }}>
                {authLoading ? 'VERIFICANDO...' : 'INGRESAR AL SISTEMA'}
              </button>
              <button type="button" onClick={() => { setOtpEnviado(false); setOtpInput(''); setMessage(''); }} style={{ width: '100%', marginTop: '0.5rem', padding: '0.6rem', backgroundColor: 'transparent', color: '#737373', border: '1px solid #262626', borderRadius: '2px', fontSize: '0.7rem', cursor: 'pointer' }}>
                Usar otro correo
              </button>
            </form>
          )}
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#000000', color: '#ededed', padding: '1.5rem', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <header style={{ borderBottom: '1px solid #262626', paddingBottom: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={handleRegistrarBiometria} style={{ backgroundColor: '#ffffff', border: 'none', color: '#000000', padding: '0.5rem 1rem', fontSize: '0.7rem', cursor: 'pointer', borderRadius: '2px', fontWeight: 'bold' }}>
            🧬 VINCULAR HUELLA
          </button>
