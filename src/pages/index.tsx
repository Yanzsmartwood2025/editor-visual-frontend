...
export default function NaylaCore() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOtpEnviado(true);
      setMessage('Código enviado. Revise su correo e ingréselo abajo.');
    } catch (err: any) {
  ...
  const handleOtpVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) { setMessage(`Código incorrecto: ${error.message}`); setAuthLoading(false); return; }
        ...
        <div style={{ width: '100%', maxWidth: '400px', border: '1px solid #262626', backgroundColor: '#050505', padding: '2.5rem', borderRadius: '4px', textAlign: 'center' }}>
          ...
          {!otpEnviado ? (
            ...
            <form onSubmit={handleOtpVerify} style={{ textAlign: 'left' }}>
              <label style={{ fontSize: '0.7rem', color: '#737373', textTransform: 'uppercase', letterSpacing: '1px', display: 'block', marginBottom: '0.5rem' }}>Código recibido en {emailInput}</label>
              <input type="text" placeholder="123456" value={otpInput} onChange={(e) => setOtpInput(e.target.value)} maxLength={6} style={{ width: '100%', padding: '0.8rem', backgroundColor: '#000000', border: '1px solid #ffffff', borderRadius: '2px', color: '#ffffff', fontSize: '1.2rem', marginBottom: '1rem', textAlign: 'center', letterSpacing: '0.5rem' }} />
              <button type="submit" disabled={authLoading} style={{ width: '100%', padding: '0.8rem', backgroundColor: '#ffffff', color: '#000000', border: 'none', borderRadius: '2px', fontWeight: 'bold', fontSize: '0.75rem', letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase' }}>
