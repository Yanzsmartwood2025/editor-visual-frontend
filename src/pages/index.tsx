...
// Inicialización de la llave de Supabase del lado del cliente
...
export default function NaylaCore() {
  ...
  // LÓGICA DE AUTENTICACIÓN BIOMÉTRICA (PASSKEY / WEBAUTHN)
  ...
  // LÓGICA DE RESPALDO: MAGIC LINK POR CORREO
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(`Error de verificación: ${err.message}`);
    } finally {
      setAuthLoading(false);
    }
  };

  // PERSISTENCIA: Cargar los cuadros automáticamente al entrar
  const cargarProyectoGuardado = async (userId: string) => {
    const { data, error } = await supabase
      .from('plantillas_editor')
      .select('*')
      .eq('perfil_id', userId)
      .single();
    
    if (data && !error) {
      if (data.video_base_url) {
        setVideoTerminado(data.video_base_url);
        setVideoResultadoUrl(data.video_base_url);
      }
      if (data.coordenadas) setRects(data.coordenadas);
    }
  };

  // PERSISTENCIA: Guardar la plantilla actual en Supabase
    ...
    if (isDrawing && currentRect) {
      ...
      setRects(rects.map(r => {
        if (r.id === draggingInfo.id) {
          let newX = currentX - draggingInfo.offsetX;
          let newY = currentY - draggingInfo.offsetY;
          newX = Math.max(0, Math.min(newX, container.width - r.width));
          newY = Math.max(0, Math.min(newY, container.height - r.height));
      ...
      setRects(rects.map(r => {
        if (r.id === resizingInfo.id) {
          ...
          if (resizingInfo.corner.includes('w')) {
            ...
            newWidth = Math.max(20, r.width - deltaX);
            if (newWidth > 20) newX = currentX;
          }
          if (resizingInfo.corner.includes('n')) {
            const deltaY = currentY - r.y;
            newHeight = Math.max(20, r.height - deltaY);
            if (newHeight > 20) newY = currentY;
          }
  ...
  const processVideo = async (motorElegido: 'nube' | 'local') => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        ...
        setTimeout(() => {
          alert(data.motor === 'local_ffmpeg' ? `Procesamiento NATIVO completado.` : `Procesamiento IA completado.`);
          setProgresoCarga(0);
          setIsProcessing(false);
        }, 500);
      } else {
      ...
      setProgresoCarga(0);
      setIsProcessing(false);
      alert("Error crítico de transmisión.");
    }
  };
  ...
  // =========================================================================
  // PANTALLA DE CONTROL DE ACCESO (SI NO HAY INICIO DE SESIÓN)
  // =========================================================================
  if (!session) {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#000000', color: '#ffffff', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', fontFamily: 'system-ui, sans-serif', padding: '2rem' }}>
        ...
        <div style={{ width: '100%', maxWidth: '400px', border: '1px solid #262626', backgroundColor: '#050505', padding: '2.5rem', borderRadius: '4px', textAlign: 'center' }}>
          ...
          <button 
            ...
            style={{ width: '100%', padding: '1rem', backgroundColor: '#ffffff', color: '#000000', border: 'none', borderRadius: '2px', fontWeight: 'bold', letterSpacing: '1px', fontSize: '0.8rem', cursor: 'pointer', marginBottom: '1.5rem', textTransform: 'uppercase' }}
          >
          ...
          <form onSubmit={handleEmailAuth} style={{ textAlign: 'left' }}>
            ...
            <input 
              ...
              style={{ width: '100%', padding: '0.8rem', backgroundColor: '#000000', border: '1px solid #262626', borderRadius: '2px', color: '#ffffff', fontSize: '0.85rem', marginBottom: '1rem' }}
            />
            <button 
              type="submit"
              disabled={authLoading}
              style={{ width: '100%', padding: '0.8rem', backgroundColor: '#000000', color: '#ffffff', border: '1px solid #ffffff', borderRadius: '2px', fontWeight: 'bold', fontSize: '0.75rem', letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase' }}
            >
  ...
  // =========================================================================
  // EL EDITOR DE SIEMPRE (SOLO SE ABRE SI YA PASÓ LA BIO/EMAIL)
  // =========================================================================
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#000000', color: '#ededed', padding: '1.5rem', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <section 
          ...
          style={{ 
            width: '100%', aspectRatio: videoTerminado ? `${videoMetadata.width}/${videoMetadata.height}` : '16/9', 
            maxHeight: '65vh', backgroundColor: '#050505', border: '1px solid #262626', borderRadius: '4px', 
            overflow: 'hidden', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative', touchAction: 'none', margin: '0 auto'
          }}
        >
          {videoTerminado ? (
            <>
              <video 
                ref={videoRef} src={videoTerminado} autoPlay loop muted 
              ...
              {!videoResultadoUrl && rects.map((r) => (
                <div 
                  ...
                  onPointerDown={(e) => { 
                    ...
                    const container = containerRef.current.getBoundingClientRect();
                    setDraggingInfo({ id: r.id, offsetX: (e.clientX - container.left) - r.x, offsetY: (e.clientY - container.top) - r.y });
                  }}
        ...
        <button 
          ...
          style={{ width: '100%', padding: '1.2rem', backgroundColor: herramientasAbiertas ? '#ffffff' : '#0a0a0a', color: herramientasAbiertas ? '#000000' : '#ffffff', border: '1px solid #262626', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', letterSpacing: '2px', textTransform: 'uppercase' }}
        >
