...
export default function NaylaCore() {
  ...
  // 1. AÑADIMOS EL PARÁMETRO "motorElegido" A LA FUNCIÓN
  const processVideo = async (motorElegido: 'nube' | 'local') => {
    if (!videoFile) return;
    
    // Si elige nube, exigimos el cuadro. Si elige local, el backend tiene coordenadas de emergencia para Meta AI
    ...
    // 2. INYECTAMOS EL MOTOR SELECCIONADO EN EL FORMDATA
      ...
      if (res.ok && data.url) {
        ...
        setTimeout(() => {
          if (data.motor === 'local_ffmpeg') {
             alert(`¡Video limpiado en servidor local exitosamente!\nMotor usado: FFMPEG Local`);
          } else {
             alert(`¡Orden enviada a la IA exitosamente!\nMotor usado: ${data.motor}\nID Tarea: ${data.prediction_id || 'N/A'}`);
          }
          setProgresoCarga(0);
      ...
      <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {herramientaActiva === 'watermark' && (
          <section style={{ backgroundColor: '#0a0a0a', padding: '1.5rem', border: '1px solid #262626', borderRadius: '8px' }}>
            ...
            {/* 3. AÑADIMOS EL CONTENEDOR CON LOS DOS BOTONES */}
            <div style={{ display: 'flex', gap: '10px' }}>
              <button 
                ...
                {isProcessing ? `Cargando... ${progresoCarga}%` : '✨ Limpiar con IA (Nube)'}
              </button>
              ...
              <button 
                ...
                style={{ 
                  flex: 1, 
                  padding: '1rem', 
                  backgroundColor: videoFile && !isProcessing ? '#10b981' : '#064e3b', 
                  color: videoFile && !isProcessing ? '#ffffff' : '#737373', 
                  border: 'none', 
                  borderRadius: '4px', 
                  cursor: videoFile && !isProcessing ? 'pointer' : 'not-allowed', 
                  fontWeight: 'bold' 
                }}>
                {isProcessing ? `Procesando...` : '🎬 Limpiar Local'}
              </button>
            </div>
