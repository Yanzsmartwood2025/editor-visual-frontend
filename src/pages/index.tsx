  ...
  {Object.keys(activeRenderJobs).length > 0 && (
    <>
      ...
      <div className={`editor-grid flex flex-col md:flex-row w-full gap-4 flex-1 overflow-hidden ${isPhoneViewport ? 'phone-video-mode' : ''} ${mobileOverlaysVisible ? 'mobile-overlays-visible' : 'mobile-overlays-hidden'} ${!isPhoneViewport && isVideoExpanded ? 'video-expanded' : ''} ${expandedPanel === 'left' ? 'left-expanded' : ''} ${expandedPanel === 'right' ? 'right-expanded' : ''}`}>
        ...
        <div ref={mainToolsCarouselRef} onScroll={() => requestAnimationFrame(() => updateCenteredToolFromScroll(mainToolsCarouselRef.current, setCenteredMainToolId))} className={`editor-maintools-bar flex gap-2 w-full p-3 ${darkMode ? 'bg-black' : 'bg-white'}`} onClick={(e) => e.stopPropagation()} style={{ overflowX: 'auto', flexWrap: 'nowrap' }}>
          {MAIN_TOOLS.map((tool) => (
