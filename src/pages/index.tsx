...
export default function NaylaCore() {
  ...
  // Estados para redimensionar y AHORA TAMBIÉN para mover el cuadro entero
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    // LÓGICA NUEVA: Mover el cuadro entero
      ...
      setRects(rects.map(r => {
        if (r.id === draggingInfo.id) {
          ...
          // Evitar que el cuadro se salga de la pantalla al moverlo
    ...
    // Lógica existente de redimensionar
  ...
  const handlePointerUp = () => {
    ...
    setDraggingInfo(null); // Soltar el cuadro
  };
      ...
      <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {videoTerminado ? (
            <>
              ...
              {!videoResultadoUrl && rects.map((r) => (
                <div 
                  key={r.id} 
                  // LÓGICA DE ARRASTRE: Inicia cuando tocas el centro del cuadro
                  onPointerDown={(e) => { 
                    e.stopPropagation(); 
                    ...
                    setDraggingInfo({
                      id: r.id,
                      offsetX: (e.clientX - container.left) - r.x,
                      offsetY: (e.clientY - container.top) - r.y
                    });
                  }}
                  style={{ 
                    position: 'absolute', left: `${r.x}px`, top: `${r.y}px`, width: `${r.width}px`, height: `${r.height}px`, 
                    border: '1px solid #ffffff', backgroundColor: 'rgba(255, 255, 255, 0.1)', pointerEvents: 'auto',
                ...
                >
                  ...
                  {/* ZONAS DE REDIMENSIÓN MÁS GRANDES PARA CELULARES (16x16 px) */}
        ...
        {videoResultadoUrl && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <p style={{ margin: 0, color: '#22c55e', fontSize: '0.85rem', textAlign: 'center', letterSpacing: '0.5px' }}>
              ✓ Video procesado. Ya puedes verlo arriba y descargarlo.
            </p>
            <a 
              href={videoResultadoUrl} 
              download
              target="_blank"
              rel="noopener noreferrer"
              style={{ display: 'block', textAlign: 'center', padding: '1rem', backgroundColor: '#171717', color: '#ffffff', border: '1px solid #262626', borderRadius: '2px', textDecoration: 'none', fontWeight: 'bold', letterSpacing: '1px', fontSize: '0.8rem' }}
            >
