  ...
  try {
    ...
    if (!plan || plan.status === 'unconfigured' || !plan.id) {
      return res.status(200).json({
        ready: false,
        message: 'Nayla Cloud todavía no tiene una ruta de imagen disponible en este entorno.',
      });
    }
    ...
    return res.status(200).json({
      ...
      jobId: plan.id,
      status: plan.status,
      message: 'Imagen preparada. Confirma para iniciar la generación.',
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'No se pudo preparar la generación.';
    console.error('[generar/plan] image plan failed', error);
    return res.status(500).json({
      error: /proyecto|sesión|usuario/i.test(message)
        ? message
        : 'Nayla no pudo preparar la generación en este intento.',
    });
  }
}
