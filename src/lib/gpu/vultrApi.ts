...
export const buildNaylaPcRuntimeUserData = ({
  ...
  const agent = [
    ...
    '        except Exception: pass',
    'def mark_desktop_ready():',
    '    try:',
    '        subprocess.run(["curl","-kfsS","--max-time","3","https://127.0.0.1:6080/vnc.html"],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=5)',
    "        req('POST',{'action':'desktop_ready'})",
    '        return True',
