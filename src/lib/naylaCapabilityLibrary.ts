...
export const EDITOR_LIBRARY_VERSION = '2026-09-24.3';
...
export const EDITOR_BOOKS = [
  ...
  { id: 'looks', title: 'Color, filtros, luz y capas', fields: ['assets'], assetFields: ['efecto', 'brightness', 'contrast', 'saturation', 'overlay', 'overlayIntensity', 'professionalEffects'],
    guide: 'Filtros oficiales en efecto: grayscale, sepia, vintage, cinematic, blur, glow, high-contrast, soft, cool-blue, warm, teal-orange, noir, desaturated-drama, horror-green, dreamy, faded-film, cyberpunk, moonlight, sunset, bleach-bypass, purple-night y blue-fire. efecto comparte campo con el movimiento, así que si necesitas movimiento + acabado conserva el movimiento y combina professionalEffects/brightness/contrast/saturation. Efectos profesionales: color-correction, glow, pixelate, chromatic-aberration, zoom-blur, vignette y light-leak. No inventes filtros fuera del catálogo.', options: { looks: effects.looks, overlays: effects.overlays, effects: effects.professionalEffects }, example: clip({ efecto: 'push-in', professionalEffects: [{ type: 'color-correction', intensity: 0.3 }] }) },
  ...
  { id: 'captions', title: 'Subtítulos con estilo', fields: ['subtitles'], assetFields: [],
    guide: 'Texto literal en text y tiempos globales start/end en segundos. Estilos oficiales: clean, cinematic, tiktok, karaoke, neon, glow, outline, shadow-3d, extrude-3d, glass, boxed, marker, underline, minimal-dark, gradient, retro, glitch, starlight, word-rise, pop y typewriter. tiktok/karaoke hacen seguimiento aproximado por palabra cuando solo se entrega un bloque; los demás respetan saltos de línea del texto. position: top/center/bottom; fontSize 20–120. color, accentColor y backgroundColor son opcionales. Fuentes disponibles: Arial, Roboto, Montserrat, Playfair Display o custom + fontUrl. No inventes nombres fuera del catálogo.', options: effects.captions, example: { ...clip({}), subtitles: [{ text: 'Texto que aparecerá\\nen pantalla.', start: 0, end: 5, style: 'extrude-3d', position: 'center', fontSize: 64, color: '#ffffff', accentColor: '#7dd3fc' }] } },
