...
type GsapMotionPreset = 'fade' | 'slide-left' | 'slide-right' | 'slide-up' | 'slide-down' | 'zoom-in' | 'zoom-out' | 'bounce' | 'elastic' | 'spin' | 'swing';
type GsapMotion = { enter?: GsapMotionPreset; exit?: GsapMotionPreset; enterDuration?: number; exitDuration?: number; intensity?: number; };
type TimelineItem = { id: string; mediaId: string; tipo: 'foto' | 'video' | 'audio'; nombre: string; etiqueta: string; url: string; durationInSeconds?: number; originalDurationInSeconds?: number; volume?: number; fadeIn?: number; fadeOut?: number; scale?: number; delay?: number; startFrom?: number; trimBefore?: number; trimAfter?: number; loop?: boolean; playbackRate?: number; transitionDuration?: number; transitionType?: 'fade' | 'none' | 'wipe' | 'slide' | 'zoom' | 'film-burn' | 'blur-slide' | 'cross-zoom' | 'dreamy-zoom' | 'linear-blur' | 'push-cut'; efecto?: string; brightness?: number; contrast?: number; saturation?: number; overlay?: string; overlayIntensity?: number; professionalEffects?: ProfessionalEffect[]; motionBlur?: { shutterAngle?: number; samples?: number }; gsapMotion?: GsapMotion; };
...
const getGsapMotionVars = (
  preset: GsapMotionPreset,
  intensity: number,
  exiting: boolean
): Record<string, number> => {
  const distance = 120 * intensity;
  const scaleDistance = 0.38 * intensity;
  ...
  switch (preset) {
    ...
    case 'slide-down':
      return { y: distance, opacity: 0 };
    case 'zoom-in':
      return { scale: exiting ? 1 + scaleDistance : Math.max(0.25, 1 - scaleDistance), opacity: 0 };
    case 'zoom-out':
      return { scale: exiting ? Math.max(0.25, 1 - scaleDistance) : 1 + scaleDistance, opacity: 0 };
    case 'bounce':
      return { y: exiting ? distance * 0.75 : -distance, opacity: 0 };
    case 'elastic':
      return { scale: exiting ? Math.max(0.25, 1 - scaleDistance) : Math.max(0.25, 1 - scaleDistance), opacity: 0 };
    case 'spin':
      return { rotation: (exiting ? 55 : -55) * intensity, scale: Math.max(0.4, 1 - scaleDistance * 0.6), opacity: 0 };
    case 'swing':
...
const GsapMotionFrame: React.FC<{
  clip: TimelineItem;
  durationInFrames: number;
  children: React.ReactNode;
  ...
  const enterDuration = Math.min(
    ...
    Math.max(0.1, clipDurationSeconds)
  );
  ...
  const scope = useGsapTimeline<HTMLDivElement>(
    ({ timeline, selector }) => {
      const target = selector('[data-gsap-motion-target]');
      ...
      if (enter) {
        ...
        timeline.from(
          target,
          {
            ...getGsapMotionVars(enter, intensity, false),
      ...
      if (exit) {
        timeline.to(
          target,
          {
            ...getGsapMotionVars(exit, intensity, true),
  ...
  return (
    <AbsoluteFill ref={scope} style={{ overflow: 'hidden' }}>
      <div
        data-gsap-motion-target
        style={{
          position: 'absolute',
          inset: 0,
            ...
            <TransitionSeries.Sequence key={clip.id} durationInFrames={clip.durationInFrames}>
              <ClipWithFades clip={clip} durationInFrames={clip.durationInFrames}>
                <MaybeMotionBlur clip={clip}>
                  {clip.tipo === 'video' ? (
                    <GsapMotionFrame clip={clip} durationInFrames={clip.durationInFrames}>
