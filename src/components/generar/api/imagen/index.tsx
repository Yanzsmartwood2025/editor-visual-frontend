...
export default function ApiImagenModule({ context }: GenerarModuleProps) {
  ...
  const prepare = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload?.error || 'No se pudo preparar la imagen.');

      if (!payload?.ready || !payload?.jobId) {
        setJobId(null);
        setPhase('failed');
        setMessage(payload?.message || 'La generación de imagen todavía no está disponible.');
        return;
      }
  ...
  const applyJob = (next: JobState) => {
    ...
    if (next.status === 'completed') {
      ...
      setPhase('running');
      setMessage(next.status === 'queued' ? 'Trabajo en cola…' : 'Nayla Cloud está generando la imagen…');
    }
  };
  ...
  const poll = async (id: string, controller: AbortController) => {
    ...
    if (!controller.signal.aborted) {
      setPhase('failed');
      setMessage('La generación sigue tardando más de lo esperado. El trabajo puede continuar en segundo plano.');
    }
  };
  ...
  return (
    <section data-generar-module="imagen" className="generar-module-stage">
      <div className="generar-stage-inner">
        <div className="generar-stage-heading">
          <span className="generar-eyebrow">API · IMAGEN</span>
          <h2>Generar imagen</h2>
          <p>Describe la imagen. Primero se prepara el trabajo y solo se ejecuta después de tu confirmación.</p>
        </div>

        <div className="generar-glass-panel">
          <label className="generar-form-label" htmlFor="generar-image-prompt">DESCRIPCIÓN</label>
          <textarea
            id="generar-image-prompt"
            ...
            disabled={busy}
            placeholder="Ejemplo: retrato cinematográfico fotorrealista, luz suave, fondo oscuro…"
          />
          ...
          {result?.url && (
            <div className="generar-result">
              <img src={result.url} alt={result.nombre || 'Imagen generada'} />
