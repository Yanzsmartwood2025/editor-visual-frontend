...
export default function ApiMusicaModule({ context }: GenerarModuleProps) {
  ...
  const prepare = async () => {
    ...
    try {
      const response = await fetch('/api/generar/music-plan', {
        method: 'POST',
      ...
      if (!response.ok) throw new Error(payload?.error || 'No se pudo preparar la música.');

      if (!payload?.ready || !payload?.jobId) {
        setJobId(null);
        setPhase('failed');
        setMessage(payload?.message || 'La generación de música todavía no está disponible.');
        return;
      }
      ...
      setPhase('failed');
      setMessage(error?.message || 'No se pudo preparar la música.');
    }
  };
  ...
  const applyJob = (next: JobState) => {
    ...
    if (next.status === 'completed') {
      setPhase('completed');
      setMessage('Música lista y guardada en la Bóveda.');
    } else if (next.status === 'failed' || next.status === 'cancelled') {
      setPhase('failed');
      setMessage(next.error || 'La música no pudo generarse.');
    } else {
      setPhase('running');
      setMessage(next.status === 'queued' ? 'Trabajo en cola…' : 'Nayla Cloud está generando la música…');
    }
  };
  ...
  const poll = async (id: string, controller: AbortController) => {
    ...
    while (!controller.signal.aborted && Date.now() - startedAt < maxMs) {
      ...
      if (!next) throw new Error('Nayla no devolvió el estado de la música.');
      applyJob(next);
    ...
    if (!controller.signal.aborted) {
      setPhase('failed');
      setMessage('La música sigue tardando más de lo esperado. El trabajo puede continuar en segundo plano.');
    }
  };
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!response.ok && response.status !== 422) {
        throw new Error(payload?.error || 'No se pudo iniciar la música.');
      }
      ...
      if (controller.signal.aborted) return;
      setPhase('failed');
      setMessage(error?.message || 'La generación de música se interrumpió.');
    }
  };
    ...
    <section data-generar-module="musica" className="generar-module-stage">
      <div className="generar-stage-inner">
        <div className="generar-stage-heading">
          <span className="generar-eyebrow">API · MÚSICA</span>
          <h2>Generar música</h2>
          <p>Describe la atmósfera, instrumentos, energía y estilo. Nayla prepara el trabajo antes de consumir el motor.</p>
        </div>

        <div className="generar-glass-panel">
          <label className="generar-form-label" htmlFor="generar-music-prompt">DESCRIPCIÓN</label>
          <textarea
            id="generar-music-prompt"
            ...
            disabled={busy}
            maxLength={3000}
            placeholder="Ejemplo: rock alternativo oscuro, guitarras amplias, batería contenida, tensión emocional y final ascendente…"
          />
          ...
          <div className="generar-action-row">
            {phase !== 'awaiting' && phase !== 'completed' ? (
              ...
              >
                {phase === 'planning' ? 'PREPARANDO…' : 'PREPARAR MÚSICA'}
              </button>
            ) : null}
          ...
          {result?.url && (
            <div className="generar-result generar-audio-result">
              ...
              <div className="generar-result-meta">
                <span>{result.etiqueta || 'MÚSICA'}</span>
