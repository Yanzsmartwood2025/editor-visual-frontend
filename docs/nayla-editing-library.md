...
`node scripts/audit-remotion.mjs` produces `docs/remotion-capabilities-audit.json`. It checks every declared Remotion dependency, installed version and static import references. Current inventory: 38 installed, none missing, 32 statically referenced, 6 without direct static references. This is not evidence that every exported API is exposed. The package-by-package status and technical exceptions are documented in `remotion-package-status.md`.
...
## Selective consultation and private reference memory

`naylaCapabilityLibrary.ts` supplies seventeen versioned books covering the current editor contract: montage, image movement, transitions, looks/layers, audio, captions, titles, clip animation/blur, procedural animation, Skia, Lottie/Rive and GLB scenes. Each book contains supported options, explanations, limits, a schema-derived contract fragment and a validated example. These document existing connected functionality; they do not activate all exports of the installed Remotion packages.

A bounded semantic routing call receives the small index, recent conversation and previous agreement summary. It selects books and updates the summary, distinguishing chosen decisions from suggestions. The main response receives selected books. Exploration omits the full timeline schema; preparation includes it. If routing fails, the main response gets the complete library instead of being restricted by a stale selection. This extra routing call has cost and latency: net token savings are not yet measured. The code contains the versioned technical manuals; no external documentation search is required on each request.

The agreement summary is stored with assistant messages and carried into subsequent turns. Chat retrieval now requests the latest thirty messages rather than the earliest thirty, and preserves up to 12,000 characters per prompt-history message. This is a model-written summary plus bounded history, not guaranteed indefinite verbatim memory of every earlier instruction. Literal on-screen text must still be checked against original content and the review table.

Reference retrieval reuses existing `nayla_action_plans` and `nayla_action_items` tables, restricted to the same user and project, current catalog version and accepted/dispatched plans. Among the latest thirty completed plans, it chooses up to two by shared capability categories. It extracts only style controls and excludes source URLs, media labels and written text. It is category-based reference retrieval, not semantic vector search or model training. Old timings in audio curves must be recalculated. Accepted plans are explicitly not advertised as successful renders. Rejected, pending, mismatched-version and unrelated-category plans are excluded. Acceptance now preserves catalog/render-context metadata when recording dispatch.

No new database table or production data mutation was needed. Read-only verification confirmed existing columns; zero current-version accepted plans existed before deployment. Memory will start accumulating when this version is deployed and plans are accepted. Recovery, current schema validation and the review/accept boundary remain in place. No new model or production setting was selected.


## Additional connected layers

The new fonts, stickers, annotations, SVG and SFX books document the actual decoration controls. Emoji, GIF, notation, rounded text boxes, progressive SVG paths, SVG extrusion, starburst and library sounds are validated, reviewed, persisted and passed through to MainComposition. Caption/title fonts survive acceptance. A separate Player preview supplements the native source/final-video clock. Package-by-package roles and the six exceptions are in [remotion-package-status.md](remotion-package-status.md); this supersedes the older coverage snapshot above.
